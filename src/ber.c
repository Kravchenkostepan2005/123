...
int ber_encode_integer(ber_buf_t *b, int64_t value)
{
	...
	int i = 0;
	// Two's complement minimal bytes
	do {
...
int ber_start_sequence(ber_buf_t *b, size_t *len_pos)
{
	...
	b->data[b->len++] = 0x30; // SEQUENCE
	*len_pos = b->len;
	b->data[b->len++] = 0x80; // placeholder; will fix on end
	return 0;
}
...
int ber_end_sequence(ber_buf_t *b, size_t len_pos)
{
	...
	size_t start = len_pos + 1; // assuming short form reserved; we will shift if needed
	size_t content_len = b->len - start;
	uint8_t lenbuf[9];
	size_t l = encode_length(lenbuf, content_len);
	// Move content forward/backward to fit length bytes
	if (l == 1) {
		b->data[len_pos] = lenbuf[0];
		return 0;
	}
	// Need to insert (l - 1) bytes before content
