...
int main(int argc, char **argv)
{
	...
	for (;;) {
		for (size_t i = 0; i < count; ++i) {
			...
			double value = 0.0;
			if (vb.type == SNMP_TYPE_INTEGER || vb.type == SNMP_TYPE_TIMETICKS) {
				value = (double)vb.int_value;
			} else if (vb.type == SNMP_TYPE_OCTET_STRING) {
				// try to parse numeric
				char tmp[128]; size_t n = vb.str_len < sizeof(tmp)-1 ? vb.str_len : sizeof(tmp)-1; memcpy(tmp, vb.str_value, n); tmp[n] = '\0';
				value = atof(tmp);
			} else {
