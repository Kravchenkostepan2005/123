...
#define SEM_GET_IDENT_CSTR(tok) \
    (((tok) && ((tok)->type == IDENTIFIER || (tok)->type == GLOBAL_IDENTIFIER || (tok)->type == STRING_TOKEN)) \
        ? (tok)->data.characters : NULL)
#endif
...
static int token_equals(const Token *tok, const char *s) {
    if (!tok || !s) return 0;
    const char *id = SEM_GET_IDENT_CSTR((Token *)tok);
    if (!id) return 0;
...
SemError check_prolog_and_rules_ex(Node *program_root, FILE *errout, IfjSemPrologRuleReason *reason_out) {
    ...
    if (class_count != 1) {
        if (errout) fprintf(errout, "SEMANTICS: Expected exactly one CLASS_NT, got %d\n", class_count);
        if (reason_out) *reason_out = IFJ_SEM_PR_CLASS_COUNT;
        return IFJ_SEM_OTHER;
    }
    Node *klass = first_direct_child(program_root, CLASS_NT);

    if (SEMANTICS_CHECK_CLASS_NAME && klass) {
        for (Node *t = klass->left_child; t != NULL; t = t->right_sibling) {
            if (t->nonterminal == NaN && t->token && (t->token->type == IDENTIFIER)) {
                if (!token_equals(t->token, "Program")) {
