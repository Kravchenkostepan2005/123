...
// Forward declarations for internal helpers used before they are defined
static int analyze_node(SemanticContext *context, Node *node);
static int analyze_class(SemanticContext *context, Node *node);
static int analyze_method(SemanticContext *context, Node *node, const char *class_name);
static int analyze_getter(SemanticContext *context, Node *node, const char *class_name);
static int analyze_setter(SemanticContext *context, Node *node, const char *class_name);
static int analyze_expression(SemanticContext *context, Node *node);

// strdup is not part of ISO C; provide a small replacement
 ...
 */
static int count_parameters(Node *node) {
    if (!node) return 0;
...
static int analyze_parameter_count(const char *func_name, int expected_count, int actual_count) {
    (void)func_name; // unused in this implementation
 ...
 */
static DataType get_token_data_type(Token *token) {
 ...
 */
static int check_literal_type_compatibility(Token_type operation, DataType left_type, DataType right_type) {
 ...
 */
static int analyze_function_call(SemanticContext *context, const char *func_name, Node *args_node) {
...
static int analyze_method(SemanticContext *context, Node *node, const char *class_name) {
    ...
    symbol->name = xstrdup(method_name);
...
static int analyze_getter(SemanticContext *context, Node *node, const char *class_name) {
    ...
    symbol->name = xstrdup(getter_name);
...
static int analyze_setter(SemanticContext *context, Node *node, const char *class_name) {
    ...
    symbol->name = xstrdup(setter_name);
